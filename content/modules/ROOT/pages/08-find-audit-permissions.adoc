...
First, from the *>_ root* terminal, check for files that have the permission level __755__.
...
Switch to the *>_ rhel* terminal.

Run this command to check if the `rhel` account can read the file `contract01.txt`:
...
This step is based on information from the https://www.redhat.com/sysadmin/audit-permissions-find[Enable Sysadmin article on the topic^]. Head there for more information on increasing the specificity of what `find` returns.
