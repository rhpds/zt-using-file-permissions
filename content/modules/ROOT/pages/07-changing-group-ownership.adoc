...
NOTE: Some people prefer the `*chgrp*` command for changing groups as it has identical syntax to `*chown*`, you do not have to add a colon when using `*chgrp*`. Others prefer to use `*chown*` with a colon so that they do not have to remember two different commands.
